...
import { useCreateCouponMutation } from 'app/features/Coubons/Coubons';
...
export default function AddCouponForm({  onClose }: AddCouponFormProps) {
  ...
  const onSubmit: SubmitHandler<AddCouponFormValues> = async (data) => {
    try {
      ...
      toast.success(t('couponAdded', { defaultValue: 'Coupon added successfully' }));
      onClose();
    } catch {
      toast.error(t('couponAddError', { defaultValue: 'Failed to add coupon' }));
    }
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      >
        ...
        <TextField
          ...
          helperText={errors.min_order_total ? t('required', { defaultValue: 'Required' }) : undefined}
          fullWidth
          size="small"
        />
        <TextField
        ...
        <Controller
          ...
          render={({ field }) => (
            <Box sx={{ minWidth: 0 }}>
              <InputLabel htmlFor="add-coupon-status" shrink sx={{ fontSize: 14, mb: 0.5 }}>
                {t('status')}
              </InputLabel>
              <select
                id="add-coupon-status"
                {...field}
                value={field.value ?? ''}
      ...
      <Stack direction="row" spacing={1} justifyContent="flex-end" sx={{ pt: 2 }}>
        ...
        <Button type="submit" variant="contained" color="primary" disabled={isSubmitting}>
          {isSubmitting ? t('adding', { defaultValue: 'Adding…' }) : t('addCoupon', { defaultValue: 'Add coupon' })}
        </Button>
      </Stack>
    </form>
  );
